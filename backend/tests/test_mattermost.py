...
class TestMattermostClient:
    ...
    def test_format_late_alert(self, sample_check):
        ...
        assert "View Check Details" in attachment["text"]
    ...
    async def test_send_late_alert_success(self, sample_check):
        ...
        with patch("app.integrations.mattermost.assert_webhook_url_safe") as mock_validate, \
            ...
            mock_validate.assert_called_once_with(client.webhook_url)
    ...
    @pytest.mark.asyncio
    async def test_send_alert_revalidates_unsafe_url_before_send(self, sample_check):
        """Unsafe webhook URLs must be refused immediately before send."""
        client = MattermostClient("http://localhost:3000/hooks/webhook")

        with patch("app.integrations.mattermost.assert_webhook_url_safe", side_effect=Exception("blocked")), \
             patch("httpx.AsyncClient") as mock_client:
            result = await client.send_late_alert(sample_check, "Test Team")
