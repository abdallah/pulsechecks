...
class TestValidateOIDCToken:
    ...
    def test_audience_with_trailing_slash(self):
        ...
        token = _create_test_token(private_key_pem, audience="https://myservice-123456.a.run.app/")
        expected_audience = "https://myservice-123456.a.run.app"

        mock_certs = {"test-key-id-1": public_pem}

        with patch('app.oidc_validator._get_google_certs', return_value=mock_certs):
            claims = validate_oidc_token(token, expected_audience=expected_audience)

        assert claims["aud"] == "https://myservice-123456.a.run.app/"

    def test_endpoint_url_audience_alias(self):
        """Test validation when the token audience matches the exact endpoint URL."""
        private_key_pem, public_pem = _generate_test_key_pair()

        token = _create_test_token(private_key_pem, audience="https://myservice-123456.a.run.app/internal/http-poll")
        expected_audience = [
            "https://myservice-123456.a.run.app",
            "https://myservice-123456.a.run.app/internal/http-poll",
        ]
        ...
        assert claims["aud"] == "https://myservice-123456.a.run.app/internal/http-poll"
...
class TestGetCloudRunUrl:
    ...
    def test_get_cloud_run_url_missing(self, monkeypatch):
        """Test error when no URL source is available."""
    ...
    def test_get_cloud_run_url_from_request(self, monkeypatch):
        """Test deriving the audience from the incoming request."""
        monkeypatch.delenv("K_SERVICE_URL", raising=False)
        monkeypatch.delenv("CLOUD_RUN_URL", raising=False)

        request = MagicMock()
        request.base_url = "https://pulsechecks-api-prod-z5kj4psbnq-uc.a.run.app/"

        assert get_cloud_run_url(request) == "https://pulsechecks-api-prod-z5kj4psbnq-uc.a.run.app"
